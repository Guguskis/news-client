...
function SignalComponent({
  ...
  const [
    {
      data: editSignalData,
      loading: editSignalLoading,
      error: editSignalError,
    },
    editSignalExecute,
  ] = API.useCryptoApi(
    {
  ...
  const [
    {
      data: deleteSignalData,
      loading: deleteSignalLoading,
      error: deleteSignalError,
    },
    deleteSignalExecute,
  ] = API.useCryptoApi(
    {
  ...
  const [
    { data: getSignalData, loading: getSignalLoading, error: getSignalError },
    getSignalExecute,
  ] = API.useCryptoApi(
    {
      url: `/api/signals/${signal?.id}`,
  ...
  const bindSignalStateFields = (signal) => {
    if (!signal) return;
    if (signal.id) setId(signal.id);
    if (signal.leverage) setLeverage(signal.leverage);
    if (signal.symbol) setSymbol(signal.symbol);
    if (signal.isLong) setIsLong(signal.isLong);
    if (signal.channel) setChannel(signal.channel);
    if (signal.triggers) setTriggers(signal.triggers);
  };
  ...
  const onSignalCancel = () => {
    if (isSignalCreate) {
      ...
      console.log(
        "onSignalCancel Tried to press cancel button while form was neither in edit or create"
      );
    }
  };
  ...
  const onSignalEdit = () => {
    if (!isSignalEdit) {
      ...
      console.log(
        "onSignalEdit Tried to press cancel button while form was neither in edit or create"
      );
    }
  };
  ...
  function EntrySection() {
    ...
    return (
      <>
        ...
        <TableContainer
          component={Paper}
          sx={{ marginBottom: "0.5rem", padding: "0.25rem" }}
        >
          <Table size="small">
            <TableBody>{entries.map(assembleTrigger)}</TableBody>
          </Table>
        </TableContainer>
      </>
    );
      ...
      <>
        ...
        >
          <Table size="small" aria-label="a dense table">
            <TableBody>{exits.map(assembleTrigger)}</TableBody>
          </Table>
        </TableContainer>
      </>
    );
