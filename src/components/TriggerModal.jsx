...
const TriggerModal = ({
  ...
  const [
    {
      data: patchTriggerData,
      loading: patchTriggerLoading,
  ...
  const [
    {
      data: getTickersData,
      loading: getTickersLoading,
      error: getTickersError,
    },
    getTickersExecute,
  ] = API.useCryptoApi(
    {
  ...
  useEffect(() => {
    if (isOpen && signal) getTickersExecute();
  }, [isOpen, signal, getTickersExecute]);

  useEffect(() => {
    if (!getTickersLoading && getTickersData) {
      if (getTickersData.length > 0) setPrice(getTickersData[0].close);
    }
  ...
  return (
    <Modal open={isOpen} onClose={onTriggerCancel}>
      <Box sx={styles.modal}>
        <ActionsBar />
  ...
  function TriggerTypeSwitch() {
    return (
      <Stack direction="row" sx={{ alignItems: "center" }} marginBottom="1rem">
