...
function HomePage() {
  const [
    { data: currencies, loading: currenciesLoading, error: currenciesError },
    currenciesExecute,
  ] = API.useCryptoApi({
    url: "/api/currencies",
    method: "GET",
  ...
  const [
    { data: signalsData, loading: signalsLoading, error: signalsError },
    signalsExecute,
  ] = API.useCryptoApi({
    url: "/api/signals",
    method: "GET",
  ...
  function isLoadingData() {
    return (
      currenciesLoading || signalsLoading || currenciesError || signalsError
    );
  }
  ...
  return (
    <Box sx={styles.body}>
      ...
      {isLoadingData() ? <CircularProgress /> : signals.map(assembleSignal)}
    </Box>
  );
}
